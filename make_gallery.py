    ...
    with open(path, newline="", encoding="utf-8") as fh:
        ...
        for row in reader:
            ...
            if matched_column and (row.get(matched_column) or "").strip():
                ...
                elif via == "inaturalist":
                    name += "  (genus placed from iNaturalist)"
                elif via == "fuzzy":
            ...
            if source and source != "taxonworks" and TW_NAME in values \
                    and "iNaturalist" not in values[TW_NAME]:
                values[TW_NAME] += f"  (from {source})"
...
function show(rowIndex) {
  ...
  reveal.onclick = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        // Anything but our own endpoint answering: an older serve.py still
        // running, or some other server. Saying "no file manager" would be a
        // guess about the wrong machine.
        reveal.textContent = "restart serve.py to enable this";
      } else {
        const outcome = (await response.text()).trim();
        reveal.textContent = {
          "ok": "opened, files selected",
          "folder-only": "opened the folder (no file manager to select with)",
          "no-manager": "no file manager installed",
          "no-files": "those files are not in media/",
        }[outcome] || outcome;
      }
    } catch {
      reveal.textContent = "open the gallery with serve.py for this";
    }
...
def main():
    ...
    if args.serve:
        # Run the launcher that was just written rather than a second copy of
        # the same logic: it already replaces a stale instance holding the port.
        subprocess.run([sys.executable, launcher, str(args.port or 8000)])
    elif args.open:
